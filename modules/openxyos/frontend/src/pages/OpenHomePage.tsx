...
export default function OpenHomePage() {
  ...
  const [providerId, setProviderId] = useState<ExperienceModelId>("ollama"), [apiKey, setApiKey] = useState(""), [showKey, setShowKey] = useState(false);
  ...
  const saveModelAndEnter = async () => {
    const selected = EXPERIENCE_MODELS.find(model => model.id === providerId);
    const isLocal = Boolean(selected && "local" in selected && selected.local);
    if (!isLocal && (apiKey.trim().length < 8 || /\s/.test(apiKey.trim()))) { setLoginError(tx("请输入有效的 API Key（不能包含空格）", "Enter a valid API key without spaces.")); return; }
    ...
    try {
      ...
      const response = await authFetch("/api/settings/ai/onboarding", {
        method: "PUT",
        body: JSON.stringify({ providerId, apiKey: isLocal ? (apiKey.trim() || "ollama") : apiKey.trim() }),
      });
