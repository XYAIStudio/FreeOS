...
const MAIN_NAV: NavItem[] = [
  ...
  { path: "/agents", icon: Bot, label: "智能体定制" },
  ...
  },
  {
    ...
    icon: Gauge,
    label: "效能仪表板",
    moduleKey: "efficiency",
  },
  {
    path: "/reflections",
    icon: Brain,
    label: "反思引擎",
    moduleKey: "reflections",
  },
  {
    path: "/knowledge",
    icon: BookOpen,
...
export default function Sidebar({
  ...
  const applyOverrides = useModuleSettingsStore(
    (state) => state.applyOverrides,
  );
  ...
  useEffect(() => {
    ...
    const onMessage = (event: MessageEvent) => {
      ...
      if (
        data?.type === "freeos:module-toggles" &&
  ...
  const sidebarContent = (
    <aside
      className={`relative flex flex-col ${widthClass} h-screen bg-bg-card border-r border-border shrink-0 transition-all duration-200`}
    >
      ...
      <div
        className={`flex items-center ${collapsed ? "justify-center px-2" : "gap-3 px-4"} py-3 border-b border-border`}
      >
        <div className="w-10 h-10 rounded bg-primary flex items-center justify-center text-white text-sm font-bold shrink-0">
          XY
        </div>
        {showText && (
        ...
        {BOTTOM_NAV.map((item) => {
          ...
          return (
            ...
            >
              <item.icon size={16} className="shrink-0" />
              {showText && <span>{item.label}</span>}
              {showText && isSuperAdmin && item.path === "/settings" && (
                <span className="ml-auto text-[9px] px-2 py-1 rounded bg-red-100 text-red-600 font-medium">
                  SA
                </span>
              )}
            </button>
      ...
      {user && (
        <div
          className={`${collapsed ? "px-2" : "px-3"} py-3 border-t border-border`}
        >
          <div
            className={`flex items-center ${collapsed ? "justify-center" : "gap-2"}`}
          >
            <div
              className={`w-8 h-8 rounded flex items-center justify-center text-white text-xs font-bold shrink-0 ${user.role === "super_admin" ? "bg-red-500" : user.role === "admin" ? "bg-orange-500" : "bg-primary"}`}
            ...
            {showText && (
              <>
                ...
                <button
                  onClick={logout}
                  className="text-text-muted hover:text-danger text-xs px-2 py-1 rounded hover:bg-bg"
                  title="退出"
                >
                  ⏻
                </button>
  ...
  return (
    <>
      ...
      <div
        className={`md:hidden fixed inset-y-0 left-0 z-50 w-56 transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        {sidebarContent}
      </div>
