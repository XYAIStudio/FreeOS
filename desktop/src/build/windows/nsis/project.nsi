...
LangString OPENXYOS_WORKDIR ${LANG_SIMPCHINESE} "组织能力在 FreeOS 宿主内运行；openXYOS Node 运行包可选"
LangString OPENXYOS_WORKDIR ${LANG_ENGLISH} "Organization runs in the FreeOS host; the openXYOS Node runtime is optional"
LangString OPENXYOS_COPY_ZIP ${LANG_SIMPCHINESE} "复制可选的 openXYOS 运行包（不解压、不启动）"
LangString OPENXYOS_COPY_ZIP ${LANG_ENGLISH} "Copy the optional openXYOS runtime payload (do not extract or start)"
LangString OPENXYOS_OPTIONAL_SKIP ${LANG_SIMPCHINESE} "跳过 openXYOS 解压与 livez。安装继续；组织页使用宿主内能力。"
LangString OPENXYOS_OPTIONAL_SKIP ${LANG_ENGLISH} "Skipping openXYOS extract and livez. Setup continues; Organization uses the in-host module."
LangString OPENXYOS_OPTIONAL_ABSENT ${LANG_SIMPCHINESE} "本安装包未包含 openXYOS 运行包。安装继续；组织页使用宿主内能力。"
LangString OPENXYOS_OPTIONAL_ABSENT ${LANG_ENGLISH} "This package has no openXYOS runtime zip. Setup continues; Organization uses the in-host module."
