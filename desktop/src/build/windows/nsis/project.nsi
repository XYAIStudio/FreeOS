...
LangString OPENXYOS_COPY_ZIP ${LANG_ENGLISH} "Copying the transitional openXYOS runtime payload"
LangString OPENXYOS_OPTIONAL_SKIP ${LANG_SIMPCHINESE} "组织能力在 FreeOS 宿主内运行（不捆绑 Node）。托管 Node + iframe 是过渡桥。安装继续。"
LangString OPENXYOS_OPTIONAL_SKIP ${LANG_ENGLISH} "Organization runs in-host (no bundled Node). Managed Node + iframe is a transition bridge. Setup continues."
LangString OPENXYOS_OPTIONAL_ABSENT ${LANG_SIMPCHINESE} "未包含 openXYOS 运行包。组织能力在 FreeOS 宿主内运行。安装继续。"
LangString OPENXYOS_OPTIONAL_ABSENT ${LANG_ENGLISH} "No openXYOS runtime zip in this installer. Organization runs in-host. Setup continues."
