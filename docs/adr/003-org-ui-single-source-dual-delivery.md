...
>
> **Current (canonical):** [product-contract.md](../product-contract.md). Dual identity: FreeOS local auth ≠ organization-module test auth. Organization identity is **not** the authority over FreeOS host identity. Managed Node and full-App iframe are a **transition bridge**. End state: migrate openXYOS into the host. Phase-5 zero-Node is a lean default, not a claim that unmigrated App surfaces are done.
>
...
组织模块代码是**唯一事实来源**（页面/能力对齐，不是身份权威）。openXYOS 网页应变成 **FreeOS Dashboard `/organization/...` 下的子 UI**，并最终能 **导出** 独立 openXYOS 系统源码。默认一个安装器、一个 Python 进程；Node sidecar / iframe 仅作过渡桥。FreeOS 软件用户本地认证与组织模块测试用户分离。**不**用 openXYOS Chat 替换 FreeOS/Octop 的 Agent 对话运行时。
...
2. **FreeOS 为产品宿主** — 统一生命周期与同域交付。**Current：** 把能力做成宿主原生部分；托管 Node 只是桥。组织模块用户体系独立于 FreeOS 软件用户，不是宿主身份权威。
...
| ~~组织身份为唯一权威~~ | **Historical / void.** 组织模块是独立测试用户体系；FreeOS 软件用户是宿主本地认证。组织身份不是宿主权威。 |
...
- `org-ui` takes a **`ShellAdapter` / `IdentityBridge`**: embedded host mode uses the FreeOS software-user session; the organization-module test environment keeps its **own** users (dual identity — see [product-contract.md](../product-contract.md)). Standalone export uses local JWT. Default UX is **no dual login wall on the host chrome**.
