...
> **Historical vs Current.** **Current：** [产品契约](product-contract.zh-CN.md)。导出可商业化的 openXYOS **系统源码** 是目标；本页描述的 `export-standalone`（SPA + 反代到宿主）是早期交付，不是完成品。Phase-5 默认零 Node 与可选 sidecar/iframe 都是过渡桥。组织模块测试用户 ≠ FreeOS 软件用户。本文件不改导出实现（那是后续项）。
...
浏览器打开 `http://127.0.0.1:3780`。**Historical / 当前实现：** 登录打 FreeOS `/api/auth/login`。**Current 契约：** 导出站最终应能带上独立的组织账号体系；FreeOS 软件用户与组织测试用户不是同一套。不要把「用 FreeOS 用户登录独立站」写成终态身份模型。
