"""``freeos org export-standalone`` writes the shared Announcements + Org + Employees seam."""
...
def test_export_standalone_lists_announcements_org_and_employees(tmp_path: Path) -> None:
    out = tmp_path / "openxyos-web"
    runner = CliRunner()
    result = runner.invoke(cli, ["org", "export-standalone", "--out", str(out)])
    assert result.exit_code == 0, result.output
    payload = json.loads(result.output)
    assert payload["modules"] == ["announcements", "organization", "employees"]
